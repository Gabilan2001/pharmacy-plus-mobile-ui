...
export default function MedicinesScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const pharmacies: Pharmacy[] = await apiRequest('GET', `/pharmacies?ownerId=${currentUser.id}`);
      // ADDED: support _id fallback so IDs are always found
    ...
    <ScrollView style={styles.container}>
      {myMedicines.map((medicine) => (
        <View key={(medicine as any).id || (medicine as any)._id} style={styles.medicineCard}>
          <Image
            source={{ uri: (medicine as any).image }}
          ...
          <View style={styles.medicineInfo}>
            ...
            {/* ADDED: show manufacturer & expiry so cart/checkout can pick these required fields */}
            ...
            <View style={styles.medicineFooter}>
              ...
              <Text style={styles.medicinePrice}>${safePrice((medicine as any).price)}</Text>
            </View>
          </View>
        </View>
      ))}
...
const styles = StyleSheet.create({
  ...
  // ADDED: tiny styles for the extra rows
  metaRow: {
    fontSize: 12,
    color: Colors.text.secondary,
    marginBottom: 4,
